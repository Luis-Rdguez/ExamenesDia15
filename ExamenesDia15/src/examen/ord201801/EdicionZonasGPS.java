...
public class EdicionZonasGPS extends JFrame {
	...
	public EdicionZonasGPS() {
		...
		mDatos = new DefaultTableModel() {
			{ setColumnIdentifiers( new Object[] { "Zona", "�rea", "Pto", "Latitud", "Longitud" } ); }
			@Override
			public void setValueAt(Object aValue, int row, int column) {
				if (column > 2) {
					super.setValueAt(aValue, row, column);
					Zona z = GrupoZonas.jardinesErandio.getZona( (String) mDatos.getValueAt( row, 0 ) );
					int subZona = (Integer) mDatos.getValueAt( row, 1 );
					int p = (Integer) mDatos.getValueAt( row, 2 );
					if (column == 3)
						z.getPuntosGPS().get( subZona ).get( p ).setLatitud( Double.parseDouble((String)aValue) );
					else
						z.getPuntosGPS().get( subZona ).get( p ).setLongitud( Double.parseDouble((String)aValue) );
					if (p == 0) {
						if (column == 3) {
							z.getPuntosGPS().get( subZona ).get( z.getPuntosGPS().get( subZona ).size()-1 ).setLatitud( Double.parseDouble((String)aValue) );
						}else {
							z.getPuntosGPS().get( subZona ).get( z.getPuntosGPS().get( subZona ).size()-1 ).setLongitud( Double.parseDouble((String)aValue) );
						}
					}
					calculaMapa();
				}
		...
		TableColumnModel mCol = tDatos.getColumnModel();
		mCol.getColumn(0).setPreferredWidth(40);
		mCol.getColumn(1).setPreferredWidth(30);
		mCol.getColumn(2).setPreferredWidth(30);
		mCol.getColumn(3).setPreferredWidth(120);
		mCol.getColumn(4).setPreferredWidth(120);
		...
		private void clickEnZona() {
			...
			Zona z = GrupoZonas.jardinesErandio.getZona( zonasErandio.get(val) );
			calcMoverACentroZona( z );
			calculaMapa();
			dibujaZona( val+1, z, Color.CYAN );
			lMensaje.setText( "Zona " + (val+1) + " tiene " + z.getNumSubzonas() + " subzonas y " + z.getNumPuntos() 
				+ " puntos diferentes." );
			// TAREA 3 - Cambio de datos de la tabla cuando se selecciona una zona
			Object[][] dt = new Object[ z.getNumPuntos() ][5];
			int numAr = 0;
			int numPt = 0;
			for (ArrayList<PuntoGPS> subZona : z.getPuntosGPS()) {
				for (int i = 0; i < subZona.size()-1; i++) {
					PuntoGPS punto = subZona.get(i);
					dt[numPt][0] = z.getCodigoZona();
					dt[numPt][1] = numAr;
					dt[numPt][2] = numPt;
					dt[numPt][3] = punto.getLatitud();
					dt[numPt][4] = punto.getLongitud();
					numPt++;
				}
				numAr++;
			}
			mDatos.setDataVector( dt, new Object[] { "Zona", "�rea", "Pto", "Latitud", "Longitud" } );
			TableColumnModel mCol = tDatos.getColumnModel();
			mCol.getColumn(0).setPreferredWidth(40);
			mCol.getColumn(1).setPreferredWidth(30);
			mCol.getColumn(2).setPreferredWidth(30);
